...
export const TenantInspectorModal: React.FC<ITenantInspectorModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn text-slate-100">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-3xl w-full p-6 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-slate-100">{tenant.businessName}</h2>
                <span
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-indigo-500/10 text-indigo-300 font-mono text-[10px] uppercase border border-indigo-500/20">
                  {tenant.planType} Plan
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5 font-mono">ID: {tenant.shopId} &bull; Owner: {tenant.ownerEmail}</p>
            </div>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-slate-200 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 border-b border-slate-800 py-3 shrink-0">
          <button
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              activeTab === "usage" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-slate-400 hover:bg-slate-800"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
              activeTab === "audit" ? "bg-indigo-600 text-white shadow-lg shadow-indigo-600/20" : "text-slate-400 hover:bg-slate-800"
            }`}
          >
            Tenant Audit Trail
          </button>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
                  <span className="text-[11px] text-slate-500 block mb-1">Daily Tickets</span>
                  <span className="text-base font-bold text-slate-100">{tenant.dailyTicketCount}</span>
                </div>
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
                  <span className="text-[11px] text-slate-500 block mb-1">Active Queue</span>
                  <span className="text-base font-bold text-indigo-400">{tenant.activeQueueLength}</span>
                </div>
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
                  <span className="text-[11px] text-slate-500 block mb-1">Quota Usage</span>
                  <span className="text-base font-bold text-emerald-400">{tenant.quotaUsagePercent}%</span>
                </div>
                <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
                  <span className="text-[11px] text-slate-500 block mb-1">Region</span>
                  <span className="text-base font-bold text-slate-300 uppercase">{tenant.region || "europe-west2"}</span>
                </div>
              </div>
              ...
              <div className="p-5 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-indigo-600/15 hover:bg-indigo-600/25 text-indigo-300 border border-indigo-500/30 text-xs font-semibold transition-all cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-rose-950/40 text-rose-400 border border-slate-700 text-xs font-semibold transition-all cursor-pointer ml-auto"
                  >
                    Soft Delete Tenant
                  </button>
                </div>
            ...
            <div className="space-y-4 text-xs">
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-slate-300">Quota Capacity Progress</span>
                  <span className="font-mono font-bold text-emerald-400">{tenant.quotaUsagePercent}% Used</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                  <span className="text-slate-500 text-[11px]">Database Operations / Day</span>
                  <span className="font-mono text-base font-bold text-slate-100 block">
                    {usageData?.firestoreReadCount || 4120} ops
                  </span>
                </div>
                <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                  <span className="text-slate-500 text-[11px]">Peak Concurrent Display Connections</span>
                  <span className="font-mono text-base font-bold text-slate-100 block">
          ...
          {activeTab === "audit" && (
            <div className="space-y-2 text-xs">
              {auditHistory.length === 0 ? (
                <p className="text-slate-500 text-center py-8">No historical audit records logged for this tenant.</p>
              ) : (
                auditHistory.map((log, idx) => (
                  <div key={idx} className="p-3 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-indigo-300">{log.action || log.event}</span>
                      <span className="text-[10px] font-mono text-slate-500">{new Date(log.timestamp || Date.now()).toLocaleString()}</span>
                    </div>
                    <p className="text-[11px] text-slate-400">Actor: {log.actorEmail || "system"}</p>
                    {log.reason && <p className="text-[11px] text-slate-300 italic">"{log.reason}"</p>}
                  </div>
                ))
              )}
            </div>
          ...
          <div className="fixed inset-0 z-60 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4">
              ...
              <form onSubmit={handleExecuteAction} className="space-y-4">
                ...
                {actionModal === "plan" && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1">Target Subscription Tier</label>
                    <select
                      value={newPlan}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">
                    Mandatory Audit Justification Reason <span className="text-rose-400">*</span>
                  </label>
                  <textarea
                    value={auditReason}
                    onChange={(e) => setAuditReason(e.target.value)}
                    placeholder="Provide explicit operational or compliance justification..."
                    className="w-full p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-100 h-24 focus:outline-none focus:border-indigo-500"
                    required
                  />
                </div>
