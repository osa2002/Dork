...
export function CustomerAlertModal({
  isOpen,
  onClose,
  errorMessage
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 dark:border-slate-800 text-center space-y-5 transform scale-100 transition-all animate-scaleIn relative overflow-hidden">
        ...
        <div className="absolute top-0 start-0 end-0 h-1 bg-gradient-to-r from-rose-500 via-red-500 to-rose-500" />
        
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 end-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors cursor-pointer p-1 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800"
          aria-label="Close"
        >
